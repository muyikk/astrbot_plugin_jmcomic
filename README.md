...
配置项 `show_cover` 默认开启，详情和随机指令会发送“封面 + 文字”；关闭后只发送文字，也不会下载封面。配置项 `daily_auto_cleanup` 默认开启，启用后插件会按服务器本地时间每天 04:00 清空 `downloads/`、`covers/` 和 `pdf/`；不会删除插件配置或 `option.generated.yml`。如果当时有下载、封面或 PDF 生成任务，清理会等待任务完成；不需要自动清理时可在 WebUI 中关闭。
...
| `/jm detail <漫画ID>` | `/禁漫 详情 <漫画ID>` | 展示封面并查看标题、作者、页数和标签 |
| `/jm random` | `/禁漫 随机` | 随机获取一本漫画的封面和详情 |
...
查询指定漫画的标题、作者、页数和标签。`show_cover` 开启时同时展示封面，封面会缓存在 `covers/`；关闭时只返回文字。该指令不会下载漫画正文图片或生成 PDF，封面获取失败时会自动降级为纯文字详情。
...
从全部分类中随机选择一页，再从该页随机选择一本漫画，返回与详情指令相同的漫画 ID、标题、作者、页数和标签。`show_cover` 开启时同时展示封面，关闭时只返回文字；该指令不会下载漫画正文图片或生成 PDF。
